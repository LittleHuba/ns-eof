...
FieldIterator<FlowField>::FieldIterator(FlowField &flowField, const Parameters &parameters,
...
template<class FlowField>
void FieldIterator<FlowField>::iterateBackwards() {
    ...
    if (Iterator<FlowField>::_parameters.geometry.dim == 2) {
        ...
        // Loop without lower boundaries. These will be dealt with by the global boundary stencils
        // or by the subdomain boundary iterators.
        for (int j = cellsY - 1 + _highOffset - 1; j >= 1 + _lowOffset; j--) {
            for (int i = cellsX - 1 + _highOffset - 1; i >= 1 + _lowOffset; i--) {
                _stencil.apply(Iterator<FlowField>::_flowField, i, j);
            }
        }
    }
    ...
    if (Iterator<FlowField>::_parameters.geometry.dim == 3) {
        ...
        for (int k = cellsZ - 1 + _highOffset - 1; k >= 1 + _lowOffset; k--) {
            for (int j = cellsY - 1 + _highOffset - 1; j >= 1 + _lowOffset; j--) {
                for (int i = cellsX - 1 + _highOffset - 1; i >= 1 + _lowOffset; i--) {
                    _stencil.apply(Iterator<FlowField>::_flowField, i, j, k);
                }
...
void GlobalBoundaryIterator<FlowField>::iterate() {
    ...
    if (Iterator<FlowField>::_parameters.geometry.dim == 2) {
        ...
        if (Iterator<FlowField>::_parameters.parallel.rightNb < 0) {
            for (int j = _lowOffset; j < Iterator<FlowField>::_flowField.getCellsY() + _highOffset; j++) {
                _rightWallStencil.applyRightWall(Iterator<FlowField>::_flowField,
                                                 Iterator<FlowField>::_flowField.getCellsX() + _highOffset - 1, j);
            }
        }
        ...
        if (Iterator<FlowField>::_parameters.parallel.topNb < 0) {
            for (int i = _lowOffset; i < Iterator<FlowField>::_flowField.getCellsX() + _highOffset; i++) {
                _topWallStencil.applyTopWall(Iterator<FlowField>::_flowField, i,
                                             Iterator<FlowField>::_flowField.getCellsY() + _highOffset - 1);
            }
        }
    }
    ...
    if (Iterator<FlowField>::_parameters.geometry.dim == 3) {
        ...
        if (Iterator<FlowField>::_parameters.parallel.topNb < 0) {
            for (int i = _lowOffset; i < Iterator<FlowField>::_flowField.getCellsX() + _highOffset; i++) {
                for (int k = _lowOffset; k < Iterator<FlowField>::_flowField.getCellsZ() + _highOffset; k++) {
                    _topWallStencil.applyTopWall(Iterator<FlowField>::_flowField, i,
                                                 Iterator<FlowField>::_flowField.getCellsY() + _highOffset - 1, k);
                }
            }
        }
        ...
        if (Iterator<FlowField>::_parameters.parallel.backNb < 0) {
            for (int i = _lowOffset; i < Iterator<FlowField>::_flowField.getCellsX() + _highOffset; i++) {
                for (int j = _lowOffset; j < Iterator<FlowField>::_flowField.getCellsY() + _highOffset; j++) {
                    _backWallStencil.applyBackWall(Iterator<FlowField>::_flowField, i, j,
                                                   Iterator<FlowField>::_flowField.getCellsZ() + _highOffset - 1);
                }
            }
        }
    }
...
void ParallelBoundaryIterator<FlowField>::iterate() {
    ...
    if (Iterator<FlowField>::_parameters.geometry.dim == 2) {
        ...
        if (Iterator<FlowField>::_parameters.parallel.rightNb >= 0) {
            for (int j = _lowOffset; j < Iterator<FlowField>::_flowField.getCellsY() + _highOffset; j++) {
                _stencil.applyRightWall(Iterator<FlowField>::_flowField,
                                        Iterator<FlowField>::_flowField.getCellsX() + _highOffset - 1, j);
            }
        }
        ...
        if (Iterator<FlowField>::_parameters.parallel.topNb >= 0) {
            for (int i = _lowOffset; i < Iterator<FlowField>::_flowField.getCellsX() + _highOffset; i++) {
                _stencil.applyTopWall(Iterator<FlowField>::_flowField, i,
                                      Iterator<FlowField>::_flowField.getCellsY() + _highOffset - 1);
            }
        }
    }
    ...
    if (Iterator<FlowField>::_parameters.geometry.dim == 3) {
        ...
        if (Iterator<FlowField>::_parameters.parallel.rightNb >= 0) {
            for (int j = _lowOffset; j < Iterator<FlowField>::_flowField.getCellsY() + _highOffset; j++) {
                for (int k = _lowOffset; k < Iterator<FlowField>::_flowField.getCellsZ() + _highOffset; k++) {
                    _stencil.applyRightWall(Iterator<FlowField>::_flowField,
                                            Iterator<FlowField>::_flowField.getCellsX() + _highOffset - 1, j, k);
                }
            }
        }
        ...
        if (Iterator<FlowField>::_parameters.parallel.backNb >= 0) {
            for (int i = _lowOffset; i < Iterator<FlowField>::_flowField.getCellsX() + _highOffset; i++) {
                for (int j = _lowOffset; j < Iterator<FlowField>::_flowField.getCellsY() + _highOffset; j++) {
                    _stencil.applyBackWall(Iterator<FlowField>::_flowField, i, j,
                                           Iterator<FlowField>::_flowField.getCellsZ() + _highOffset - 1);
                }
            }
        }
