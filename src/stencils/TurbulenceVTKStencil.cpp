...
inline void TurbulenceVTKStencil::apply(FlowField &flowField, int i, int j) {
    ...
    if ((flowField.getFlags().getValue(i, j) & OBSTACLE_SELF) == 0) {
        ...
        this->_wallDistanceStream << flowField.getNearestWallDistance().getScalar(i, j) << std::endl;
    } else {
        // Write viscosity to viscosity data stream
        this->_viscosityStream << 0.0 << std::endl;
        this->_wallDistanceStream << 0.0 << std::endl;
    }
}
...
inline void TurbulenceVTKStencil::apply(FlowField &flowField, int i, int j, int k) {
    ...
    if ((flowField.getFlags().getValue(i, j, k) & OBSTACLE_SELF) == 0) {
        ...
        this-> _wallDistanceStream << flowField.getNearestWallDistance().getScalar(i, j, k) << std::endl;
    } else {
        // Write viscosity to viscosity data stream
        this->_viscosityStream << 0.0 << std::endl;
        this->_wallDistanceStream << 0.0 << std::endl;
...
void TurbulenceVTKStencil::write(int timeStep, std::basic_ofstream<char> &vtkFile) {
    ...
    if (vtkFile.is_open()) {
        ...
        // Write the viscosity data to the file
        vtkFile << std::endl;
        vtkFile << "SCALARS wallDistance float 1" << std::endl;
        vtkFile << "LOOKUP_TABLE default" << std::endl;
        vtkFile << this->_wallDistanceStream.rdbuf();
        vtkFile << std::endl << std::endl;
