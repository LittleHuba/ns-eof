...
if __name__ == '__main__':
	...
	for re in reynolds:
		for step in steps:
			with open("conf_channel_turbulence_re_" + str(re) + "_x_" + str(step[0]) + "_y_" + str(step[1]) + ".xml", "w") as f:
				...
				vtk = "    <vtk interval=\"0.1\">channel_turbulence_ns-eof_re_{}_steps_{},{}</vtk>\n".format(re, step[0], step[1])
	...
	top = "#!/bin/bash\n#SBATCH -D /home/hpc/t1221/lu26xut/ns-eof #starting directory\n"
	...
	for re in reynolds:
		for step in steps:
			with open("conf_channel_turbulence_re_" + str(re) + "_x_" + str(step[0]) + "_y_" + str(step[1]) + ".job", "w") as f:
				outName = "#SBATCH -o /home/hpc/t1221/lu26xut/ns-eof/output/channel_re_{0}_x_{1}_y_{2}.out\n#SBATCH -J channel_tuburbulence_ns-eof_{0}_x_{1}_y_{2} #name shown in squeue\n#SBATCH --ntasks={3}\n".format(re, step[0], step[1], 16)
